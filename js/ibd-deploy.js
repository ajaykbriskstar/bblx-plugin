...
// API FUNCTION - Distribution
		jQuery.getJSON(apiDist, function(data){
					if (data != undefined) {
							console.log('distribution data init -', data)
							let exDate = data.slice(-1)[0].ex_date; //Ex Date
							jQuery(".ex_date").html(getDataLayout(exDate)); // Ex Date - Formatted
							console.log(getDataLayout(exDate))
							let reDate = data.slice(-1)[0].record_date; // Record Date
							jQuery(".record_date").html(getDataLayout(reDate));
							let payDate = data.slice(-1)[0].pay_date; // Pay Date
							jQuery(".pay_date").html(getDataLayout(payDate));
							let divIncomeData = data.slice(-1)[0].dividend_income; // Dividend Income
							jQuery(".dividend_income").html(divIncomeData); // Dividend Data
					}
					else {
							jQuery(".ex_date").html("Mar 31, 2022"); // Ex Date - null
							jQuery(".record_date").html("Apr 3, 2022"); // Record Date - null
							jQuery(".pay_date").html("Apr 6, 2022"); // Pay Date null
							jQuery(".dividend_income").html("0.23417"); // Dividend Data null
					}
		});
...
async function getTableData(){
    ...
    stocks_list_top_10.forEach((stock) => {
        ...
        table = table + `<tr>`;
        table = table + `<td> ${stock.etfticker}`;
    ...
    table1 += `
        ...
        chunk(stocks_list, 20).forEach((stock_list) => {
            stock_list.forEach((stock) => {
                ...
                const formatedNumber = new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(unFormatedNumber)
                table1 = table1 + `<tr>`;
                table1 = table1 + `<td> ${stock.etfticker}`;
