const apiOverview = 'https://data.etflogic.io/test?apikey=a2cabd13-a2f6-3412-a1af-13fa70eabc31&function=overview&ticker=XEMD';
//const apiFlows = 'https://data.etflogic.io/test?apikey=a2cabd13-a2f6-3412-a1af-13fa70eabc31&function=flows&ticker=XEMD';
const apiDist = 'https://data.etflogic.io/test?apikey=a2cabd13-a2f6-3412-a1af-13fa70eabc31&function=distribution&ticker=XEMD';
const apiIndex = 'https://data.etflogic.io/test?apikey=a2cabd13-a2f6-3412-a1af-13fa70eabc31&function=overview&ticker=XEMD';
...
// API FUNCTION - Overview
	...
	if (fid != null) {
		...
		jQuery(".fidX").html(fid);; // FID Data
	} else {
		jQuery(".fidX").html("NA"); // FID null
	}
	...
	let aumData = data[0].aum; // AUM
		...
		if (aumData != null) {
			//aumData = (nFormatter(24221780, 2));
		  	aumData = (nFormatter(aumData, 2));
		  	aumData = aumData.split(/(\d+)/);
			if (aumData && aumData.length > 0){
				var className = ".fundNetAssets";
				jQuery.each(aumData, function(index, value) {
					if(value == 'k' || value == 'M' || value == 'B' || value == 'T'){
						className = ".fundAssetsPost";
					}
					if(value != ""){
						jQuery(className).append(value);
					}
				});	
			}else{
			...
			jQuery(".fundNetAssets").html("NA"); // AUM null
		}
	...
	function getDataLayout(date) {
		return new Date(date.replace(/-/g, '\/')).toLocaleString('en-US', { month: 'short', day: 'numeric', year: 'numeric'});
	}
...
jQuery.getJSON(apiIndex, function(data){
	...
	let pc5 = data[0].stripped_ytm;
	if (pc5 != null) {
		pc5 = (nFormatter(data[0].stripped_ytm*100, 2));
	...
	let pc7 = data[0].stw_trsy;

	if (pc7 != null) {
		//pc7 = (nFormatter(data[0].stw_trsy*100, 2));
		pc7 = parseInt(pc7*10000);
...
async function getBarGraphData() {
	const api = 'https://data.etflogic.io/test?apikey=a2cabd13-a2f6-3412-a1af-13fa70eabc31&function=basket-exposures&ticker=XEMD';
...
async function getPieChartData() {
	...
	let filteredData = data.filter(
		obj => {
			return obj.metric_type === 'country'
		}
	);
...
async function getTableData(){
	apiAll = 'https://data.etflogic.io/test?apikey=a2cabd13-a2f6-3412-a1af-13fa70eabc31&function=holdings&ticker=XEMD';
	response = await fetch(apiAll);
	dataAll = await response.json();
	stocks_list_all = dataAll;
	apiTop = 'https://data.etflogic.io/test?apikey=a2cabd13-a2f6-3412-a1af-13fa70eabc31&function=basket-exposures&ticker=XEMD';
	...
	stocks_list_top_10 = stocks_list.filter((obj) => obj.metric_type === 'country').sort( (a, b) => b.basket_exposure.toString().localeCompare(a.basket_exposure.toString()));
	var count = 0;
...
async function getInvestmentGrowth(){
	const api = 'https://data.etflogic.io/test?apikey=a2cabd13-a2f6-3412-a1af-13fa70eabc31&function=total-return&format=json&ticker=XEMD';
	const response = await fetch(api);
	const data = await response.json();
	...
	let apidays = [],price = [],daynumber = [] ; 
	let datecounter = 0,pricecounter=0;
...
async function getPremDisDataTable(){
	...
	const api = `https://data.etflogic.io/test?apikey=a2cabd13-a2f6-3412-a1af-13fa70eabc31&function=premium-discount&ticker=XEMD&date_gteq=${lastYear.year}.${lastYear.month}.${lastYear.day}`;
	console.log(api);
	const response = await fetch(api);
	const data = await response.json();
...
const api = `https://data.etflogic.io/test?apikey=a2cabd13-a2f6-3412-a1af-13fa70eabc31&&function=performance&format=json&ticker=XEMD`;
//const api = `https://data.etflogic.io/test?apikey=a2cabd13-a2f6-3412-a1af-13fa70eabc31&&function=performance&format=json&ticker=XEMD`;
const response = await fetch(api);
const data = await response.json();
...
async function downloadCsv(){
	const api = 'https://data.etflogic.io/test?apikey=a2cabd13-a2f6-3412-a1af-13fa70eabc31&function=holdings&ticker=XEMD';
	const response = await fetch(api);
	const data = await response.json();
