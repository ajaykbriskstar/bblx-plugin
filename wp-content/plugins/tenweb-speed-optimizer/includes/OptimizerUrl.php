...
class OptimizerUrl
{
    ...
    /**
     * Returns Returns whether page should be optimized or not based on page url and fastcgi excluded pages merged with option
    ...
    public static function urlIsOptimizableWithReason($url = null, $is_admin = false)
    {
        ...
        if (isset( $server_data['HTTP_USER_AGENT'] ) && preg_match( '#(' . implode( '|', self::TWO_FASTCGI_NON_CACHED_PAGES_IF_USER_AGENT ) . ')#', $server_data['HTTP_USER_AGENT'] ) ) {
    ...
    public static function getPageModeByID( $page_id ) {
        ...
        return array ( 'id' => $id, 'type' => $type, 'page_mode' => $page_mode );
    }
}
