...
class OptimizerMain
{
    ...
    public function init()
    {
        ...
        if(TENWEB_SO_HOSTED_ON_10WEB && !is_array($two_default_mode) && !isset($_GET["mode"]) && !wp_doing_ajax()){
        ...
        if ($should_serve_optimized_page === true) {
            ...
            $request_url = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://" . sanitize_url( $_SERVER['HTTP_HOST'] ) . sanitize_url( $_SERVER['REQUEST_URI'] );
            ...
            if(!isset($_GET["two_optimize_only_images"])){
        ...
        if ( isset( $_GET[ 'two_detect_post_id' ] ) ) {
    ...
    private function end_buffering($content)
    {
        ...
        if(isset($_GET["two_optimize_only_images"]) && $_GET["two_optimize_only_images"]==="1"){
        ...
        if (!$this->cacheStructure->isFromCache()) {
            ...
            if(!empty($twoptimizeStyles->two_async_css_arr) || !empty($twoptimizeScripts->two_js_list) || !empty($this->critical_fonts_arr) || !empty($twoptimizeScripts->two_js_list_excluded)){
                ...
                $content = OptimizerUtils::inject_in_html($content, $two_worker_data_to_inject_excluded_js, array('<head', 'before'));
                ...
                $this->cacheStructure->addToTagsToAdd($two_worker_data_to_inject_excluded_js, array('<head', 'before'));
    ...
    private function set_modes($post_id){
        ...
        if((isset($_GET["two_mode"]) && !empty($_GET["two_mode"])) || (isset($_GET["two_level"]) && !empty($_GET["two_level"]))){
            if(isset($_GET["two_mode"])){
                $mode_name = sanitize_text_field( $_GET["two_mode"] );
                $mode = OptimizerUtils::get_modes($mode_name);
            }elseif (isset($_GET["two_level"])){
                $level = (int)$_GET["two_level"] ;
    ...
    private function should_serve_optimized_page($post_mode){
        ...
        if(isset($_GET["two_critical_status"]) || isset($_GET['two_action']) || isset($_GET['two_nooptimize']) || isset($_GET['elementor-preview'])) {
            return false;
        }
        ...
        if(isset($_GET['two_mode']) && $_GET['two_mode'] === 'no_optimize'){
