...
class OptimizerLogger {
    ...
    private function __construct(){
        add_action('http_api_debug', array($this, 'log_performance_requests_api_debug'), 10, 5);

        if(!wp_next_scheduled('two_daily_cron_hook')) {
            add_action('two_daily_cron_hook', array($this, 'two_daily_cron_hook'));
        }
    ...
    public static function add_critical_css_log($request_data, $newly_connected_website, $flow_id, $status_code){
        $log_data = [
        ...
        self::add_log('critical_css', $log_data);
    }
    ...
    public static function add_log($log_type, $log_data, $include_stack_trace = true){
        if (!TENWEB_SO_DEBUG_MODE) {
            return;
        }
    ...
    public function two_daily_cron_hook(){
