...
use TenWebOptimizer\OptimizerMain;
...
class OptimizerWebPageCache {
    ...
    protected function init(){
        ...
        $this->accept_gzip = isset( $_SERVER['HTTP_ACCEPT_ENCODING'] ) && strpos($_SERVER['HTTP_ACCEPT_ENCODING'], 'gzip') !== false;
    ...
    protected function serve_cached_file($file_to_serve){
        ...
        require_once TWO_PLUGIN_DIR_CACHE . '/includes/OptimizerMain.php';
        OptimizerMain::two_headers();

        // If the cache has not been modified since $_SERVER["HTTP_IF_MODIFIED_SINCE"], than send 304 not modified and
        // browser use cached html (cached in browser)
        if(!empty($_SERVER["HTTP_IF_MODIFIED_SINCE"]) && (strtotime($_SERVER["HTTP_IF_MODIFIED_SINCE"]) === $file_time)) {
            header(strip_tags( $_SERVER['SERVER_PROTOCOL'] ) . ' 304 Not Modified', true, 304);
    ...
    protected function cache_content($buffer){
        ...
        if(!is_dir($this->cache_dir)) {
            if(!mkdir($concurrentDirectory = $this->cache_dir, 0777, true) && !is_dir($concurrentDirectory)) {
                return $buffer;
            }
        }
        ...
        file_put_contents($this->cache_file_gzip, gzencode($buffer));
    ...
    protected function maybe_clear_cache($file_to_serve){
        ...
        $is_home = self::is_home(explode("?", strip_tags( $_SERVER["REQUEST_URI"] ))[0]);
    ...
    public function get_cache_file_dir(){
        $request_uri = explode("?", strip_tags( $_SERVER["REQUEST_URI"] ))[0];
        $cache_file_dir = self::get_cache_dir_for_page(strip_tags( $_SERVER["HTTP_HOST"] ), $request_uri);
