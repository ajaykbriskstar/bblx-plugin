...
class OptimizerWebPageCacheWP {
    ...
    function __construct(){
        ...
        if ( ( !isset($_GET['action']) || $_GET['action'] != 'deactivate' )
            && ( !isset($_GET['rest_route']) || $_GET['rest_route'] != '/tenweb_so/v1/set_modes' ) ) {
            add_action( "update_option_two_settings", array( $this, 'enable_disable_page_cache' ), 10, 3 );
        }
    ...
    public function enable_disable_page_cache($old_value, $value, $option){
        ...
        if ( $value === "on" ) {
            ...
            if ( !file_exists( WP_CONTENT_DIR . '/advanced-cache.php' ) ) {
                ...
                else if ( !defined('WP_CACHE' ) || !WP_CACHE || !defined("TWO_PLUGIN_DIR_CACHE" ) ) {
    ...
    public function add_wp_cache_constant(){
        ...
        if(!is_writable($this->wp_config_file_path) || !is_readable($this->wp_config_file_path) || TENWEB_SO_HOSTED_ON_10WEB) {
            return false;
        }
    ...
    public function maybe_clear_page_cache(){
        ...
        if(!empty($_GET["permalink"]) && wp_verify_nonce("two_clear_page_cache") !== null) {
            ...
            OptimizerUtils::clear_cloudflare_cache(array($permalink));
    ...
    public function transition_post_status($new_status, $old_status, $post){
        ...
        if($new_status === "publish" || $old_status === "publish") {
            OptimizerUtils::clear_cloudflare_cache(array(get_home_url()));
