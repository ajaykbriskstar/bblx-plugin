...
class OptimizerUtils
{
    ...
    public static function set_critical(){
        if(isset($_POST["token"], $_POST["page_id"]) && get_option("two_critical" . sanitize_text_field( $_POST["page_id"] )) === $_POST["token"]) {
            \TenWebWpTransients\OptimizerTransients::delete("two_critical".sanitize_text_field( $_POST["page_id"] ));
        ...
        if ($two_serve_optimized_bg_image === 'on') {
            if(isset($critical) && isset($critical->critical_bg) && !empty($critical->critical_bg)){
                ...
                if(file_exists($critical_bg_file_dir)){
                    $critical_bg_data = file_get_contents($critical_bg_file_dir);
    ...
    public static function clear_cloudflare_cache($prefixes = array()){
        global $TwoSettings;
        $cloudflare_cache_status = $TwoSettings->get_settings("cloudflare_cache_status");
        if($cloudflare_cache_status !== "on" || TENWEB_SO_HOSTED_ON_10WEB){
            return true;
        }
        $domain_id = get_site_option('tenweb_domain_id');
        $access_token = get_site_option(TENWEB_PREFIX . '_access_token');
    ...
    public static function init_flow_score_check($only_do_request = false){
        ...
        if($flow_score_check_init !== "1" && !is_array($two_flow_speed) && !\Tenweb_Authorization\Login::get_instance()->check_logged_in()){
            ...
            $desktop_score = \TenWebSC\TWScoreChecker::twsc_google_check_score(TENWEB_SO_NOOPTIMIZE_URL, "desktop");
            $mobile_score = \TenWebSC\TWScoreChecker::twsc_google_check_score(TENWEB_SO_NOOPTIMIZE_URL, "mobile");
            $speed_data["nooptimize_score"] = array();
            if(isset($desktop_score["desktop_score"], $desktop_score["desktop_tti"])){
                $speed_data["nooptimize_score"]["desktopScore"] = $desktop_score["desktop_score"];
                $speed_data["nooptimize_score"]["desktopTti"] = $desktop_score["desktop_tti"];
            }
            if(isset($mobile_score["mobile_score"], $mobile_score["mobile_tti"])){
                $speed_data["nooptimize_score"]["mobileScore"] = $mobile_score["mobile_score"];
                $speed_data["nooptimize_score"]["mobileTti"] = $mobile_score["mobile_tti"];
            }
        ...
        global $wpdb;
        $row = $wpdb->get_row( $wpdb->prepare( "SELECT option_value FROM $wpdb->options WHERE option_name = %s LIMIT 1", TENWEB_PREFIX . '_access_token' ) );
        $access_token = false;
        ...
        self::add_log_for_score_check_flow("two_access_token", $access_token);
