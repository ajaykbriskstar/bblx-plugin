...
class OptimizerImages
{
    ...
    const TWO_EXCLUDE_LAZY_KEYWORDS = array(
        'image-compare__'
    );
    /**
     * Options.
    ...
    {
        add_filter('twoptimize_html_after_minify', array($this, 'filter_lazyload_images'), 10, 1);
        add_filter('twoptimize_html_after_minify_iframe', array($this, 'filter_lazyload_iframes'), 10, 1);
        add_filter('twoptimize_html_after_minify_video', array($this, 'filter_lazyload_video'), 10, 1);
        add_filter('twoptimize_html_images', array($this, 'filter_optimize_html_images'), 10, 1);
    }
    ...
    public function add_lazyload($tag, $placeholder = '')
    {
        ...
        $two_exclude_lazyload = $this->TwoSettings->get_settings("two_exclude_lazyload");
        if (isset($two_exclude_lazyload) && !empty($two_exclude_lazyload)) {
            $exclude_lazyload = explode(",", $two_exclude_lazyload);
            foreach ($exclude_lazyload as $name) {
    ...
    public static function inject_classes_in_tag($tag, $target_class)
    {
        ...
        if (strpos($tag, 'class=') !== false) {
            ...
            } else if (strpos($tag, '<iframe') !== false) {
                $tag = str_replace('<iframe ', '<iframe class="' . trim($target_class) . '" ', $tag);
            } else if (strpos($tag, '<video') !== false) {
    ...
    public static function add_attachment_id_to_img( $content ) {
        ...
        $attach_ids = $wpdb->get_results( $sql, OBJECT_K );
