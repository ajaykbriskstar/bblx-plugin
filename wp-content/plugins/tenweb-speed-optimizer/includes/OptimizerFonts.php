...
class OptimizerFonts extends OptimizerBase
{
    ...
    private $styleExtensions = array(
        'css' => 'text/css',
    );

    private $scriptExtensions = array(
        'js' => 'application/javascript',
    );

    private $videoExtensions = array(
        'avi' => 'video/x-msvideo',
        'mpeg' => 'video/mpeg',
        'ogv' => 'video/ogg',
        'ts' => 'video/mp2t',
    ...
    {
        if (in_array($preloadType, self::TWO_PRELOAD_LINK_TYPES, false)) {
            if (!empty($links)) {
                $preloadString = '';
            ...
            if (!empty($preloadString)) {
                $this->content = OptimizerUtils::inject_in_html($this->content,$preloadString, array( '<meta', 'before' ));
            }
        }
    }
    ...
    public function combineGoogleFonts()
    {
        ...
        if(count($fonts)>1){
            ...
            $this->content = OptimizerUtils::inject_in_html($this->content,$google_font_tag, array( '<title', 'before' ));
    ...
    public function addPreconnect()
    {
        ...
        $this->content = OptimizerUtils::inject_in_html($this->content,$preconnect_string, array( '<title', 'before' ));
...
<noscript>
    ...
    public function add_WebFont(){
        $webFont_js = "
        <script data-pagespeed-no-defer " . OptimizerScripts::TWO_NO_DELAYED_JS_ATTRIBUTE . ">
            WebFontConfig = {
              google: {families: ".json_encode($this->webFont_list)."}
            };
            (function () {
        ...
        ";
        $this->content = OptimizerUtils::inject_in_html($this->content,$webFont_js, array( '<title', 'before'  ));
    }
