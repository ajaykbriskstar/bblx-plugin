    ...
    public function apiRequest($method = "GET", $request_data = array(), $query = array(), $headers = array())
    {
        try {
            ...
            if (is_wp_error($response) || $this->response_status_code !== 200 || empty($response['body'])) {
                Logs::setLog("api:" . $url . ":" . $method . ":response-error-status", wp_remote_retrieve_response_code($response));
                Logs::setLog("api:" . $url . ":" . $method . ":response-error", $response);
            ...
            Logs::setLog("api:" . $this->api_action . ":" . $method . ":exception", $e->getMessage());
        }
