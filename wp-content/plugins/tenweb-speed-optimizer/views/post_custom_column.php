...
<?php if(\TenWebOptimizer\OptimizerUrl::urlIsOptimizable($page_url)):?>
  ...
  <span data-post-id="<?php esc_attr_e( $post_id ) ?>" class="two-pages-list-reanalyzing <?php echo
        ( $reanalyze_button_status_current || $reanalyze_button_status_previous ) ? '' : 'two-hidden'; ?>">
      <span class="two-page-speed two-optimizing"></span>
      <?php esc_html_e('Reanalyzing score…', 'tenweb-speed-optimizer'); ?>
  </span>
  <span data-post-id="<?php esc_attr_e( $post_id ) ?>" class="two-wp-pages-list-tooltip two-optimized-see-more <?php echo
        ( $reanalyze_button_status_current || $reanalyze_button_status_previous ) ? 'two-hidden' : ''; ?>">
