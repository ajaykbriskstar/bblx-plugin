...
use Barn2\DLP_Lib\Util as Lib_Util;
use Exception;
...
class Frontend_Form implements Registerable, JsonSerializable {
	...
	public function register_scripts() {
		$script_dependencies = array_merge( Lib_Util::get_script_dependencies( document_library_pro(), './assets/js/dlp-submission-form.min.js' )['dependencies'], [ 'jquery' ] );

		wp_register_script( 'dlp-frontend-submission', $this->asset_url( 'js/dlp-submission-form.min.js' ), $script_dependencies, document_library_pro()->get_version(), true );
		wp_register_style( 'dlp-frontend-submission', $this->asset_url( 'css/dlp-submission-form.min.css' ), [], document_library_pro()->get_version() );
	...
	protected function upload_file( $field_key, $field ) {
		...
		if ( isset( $_FILES[ $field_key ] ) && ! empty( $_FILES[ $field_key ] ) && ! empty( $_FILES[ $field_key ]['name'] ) ) {
			if ( ! empty( $field['allowed_mime_types'] ) ) {
				...
				$allowed_mime_types = Uploader::get_allowed_mime_types();
			}
