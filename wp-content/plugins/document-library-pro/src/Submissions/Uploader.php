...
class Uploader {
	...
	public function upload_file( $file, $args = [] ) {
		...
		if ( '' === $args['allowed_mime_types'] ) {
			$allowed_mime_types = self::get_allowed_mime_types( $dlp_uploading_file );
		} else {
	...
	/**
	 * Get the list of allowed mime types for the uploader.
	 *
	 * @return array
	 */
	public static function get_allowed_mime_types() {
		 ...
		 *
		 * @param array $types The list of allowed types.
		 * @param string $field The field key for the upload.
		 */
		return apply_filters( 'dlp_mime_types', $allowed_mime_types );
	}
