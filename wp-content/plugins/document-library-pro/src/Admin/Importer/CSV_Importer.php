...
class CSV_Importer {
	 ...
	 * @return array
	 */
	public function parse_taxonomies_field( $label, $value ) {
		$taxonomy = $this->get_taxonomy_by_label( $label );
	...
	public function get_taxonomy_by_label( $label ) {
		$matches = array_filter(
			$this->taxonomies,
			function( $t ) use ( $label ) {
				return strtolower( $t->label ) === strtolower( $label );
			}
		);
	...
	protected function expand_data( $data ) {
		...
		foreach ( $data as $key => $value ) {
			if ( $this->starts_with( $key, 'cf:' ) ) {
				...
				$label      = str_replace( 'tax:', '', $key );
				$taxonomies = $taxonomies + $this->parse_taxonomies_field( $label, $value );
				unset( $data[ $key ] );
			}
		}
