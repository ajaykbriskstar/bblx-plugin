...
class Document_Grid_Html {
	...
	private function get_result_count() {
		...
		if ( $this->query->get_total_posts() !== $this->query->get_total_filtered_posts() ) {
			...
			$document_count = esc_html( sprintf( __( '%1$d documents (%2$d in total)', 'document-library-pro' ), $this->query->get_total_filtered_posts(), $this->query->get_total_posts() ) );
		} else {
			/* translators: %d: The total number of documents. */
			$document_count = esc_html( sprintf( __( '%d documents', 'document-library-pro' ), $this->query->get_total_filtered_posts() ) );
		}
