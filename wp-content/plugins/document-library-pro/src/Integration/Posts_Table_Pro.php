...
class Posts_Table_Pro implements Registerable, Service {
	...
	public function register() {
		...
		// Force Strings to Documents
		add_filter( 'document_library_pro_language_defaults', [ $this, 'format_string_totals' ], 20, 1 );
	...
	public function add_column_defaults( $column_defaults ) {
		return array_merge(
			...
			[
				...
				'file_size'      => [
					'heading'  => __( 'Size', 'document-library-pro' ),
					'priority' => 12
				],
				'file_type'      => [
					'heading'  => __( 'Type', 'document-library-pro' ),
					'priority' => 11
				],
				'download_count' => [
	...
	/**
	 * Switch 'post'|'posts' strings to 'document'|'documents'
	 *
	 * @param array $strings
	 * @return array
	 */
	public function format_string_totals( $strings ) {
		$strings_to_replace = array_filter( $strings, 'is_scalar' );

		return str_replace(
			[ '_POST_', '_POSTS_' ],
			[ __( 'document', 'document-library-pro' ), __( 'documents', 'document-library-pro' ) ],
			$strings_to_replace
		);
	}
